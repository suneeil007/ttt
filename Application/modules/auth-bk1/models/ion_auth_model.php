...
*/
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if($this->is_time_locked_out($identity))
		{
			...
			echo $this->hash_password($password); die();
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				$this->update_visit_count($user->id);
	...
	function update_visit_count($user_id){		
		$this->db->query('UPDATE users SET login_times = (login_times + 1) WHERE id ='. $user_id);		
	}
	...
	**/
	...
	function get_usercontacts($col){
		...
		$query = $this->_custom_query('SELECT '.$string.' FROM users') ;
	...
	function get_newUsers($group_id){
		$query = '';
		
		if($group_id == 6){
		$query  = $this->_custom_query('SELECT COUNT(users.id) AS new_users FROM users JOIN employer_profile ON employer_profile.user_id = users.id
AND (users.viewed_date IS NULL ||  users.viewed_date = "0000-00-00 00:00:00") ORDER BY users.created_on DESC');
		
	    }elseif($group_id == 2){
			
		$query  = $this->_custom_query('SELECT COUNT(users.id) AS new_users FROM users JOIN jobseeker_pers_info ON jobseeker_pers_info.user_id = users.id AND (users.viewed_date IS NULL ||  users.viewed_date = "0000-00-00 00:00:00") ORDER BY users.created_on DESC');		
	...
	function get_groupName($user_id){
		$query  = $this->_custom_query('SELECT groups.name FROM users JOIN users_groups ON users_groups.user_id = users.id
JOIN groups ON groups.id = users_groups.group_id WHERE users.id = '.$user_id);				

		...
		<tr class="row<?php echo $user->id ;?>" <?php if($counter%2 == 0)echo ' id="tble-row-even"' ; else echo ' id="tble-row-odd"' ;?>>
			...
			<td>
				<?php foreach ($user->groups as $group):?>
					<?php echo anchor("adm-panel/users/edit_group/".$group->id, $group->name) ;?><br />
                <?php endforeach?>
			</td>
			<td><?php echo ($user->active) ? anchor("adm-panel/users/deactivate/".$user->id, lang('index_active_link')) : anchor("adm-panel/users/activate/". $user->id, lang('index_inactive_link'));?></td>
			<td>
			<a href="<?php echo base_url().'adm-panel/users/edit_user/'.$user->id; ?>"><img src="<?php echo base_url().'assets/images/edit.png'; ?>" title="Edit" style="margin-right:15px;" /></a>
			<!--<a href="#" onclick='deleteUser(<?php echo $user->id ;?>)'><img src="<?php echo base_url().'assets/images/delete.png'; ?>" title="Delete" /></a>-->
            <?php if($user->active ==0):?>
	            <a href="#" onclick="send_verification(<?php echo $user->id ;?>)">Send code</a>            
            <?php endif ;?>
            </td>
		</tr>
	<?php endforeach;?>
</table>      <br/>
<p><span class="crudlinks" style="float:left;margin-right:20px;"><a href="#" title="dtable" id="deleteall" style="width:95px;">Delete Selected</a></span>

   <!-- <span class="crudlinks" style="float:left;"> <?php echo anchor('adm-panel/users/create_user', lang('index_create_user_link'), array('class'=>'widthanchornew'))?></span> 
    <span class="crudlinks" style="float:left;"> <?php echo anchor('adm-panel/users/create_group', lang('index_create_group_link'), array('class'=>'widthanchorgroup'))?></span>-->
</p>
...
function deleteF(id){	         
		...
		$.ajax({	
			   type: "GET",
			   url: '<?= base_url()?>adm-panel/users/delete_user/'+ id,
			   success: function(){
...
function send_verification(id){	         
	...
	if(confirm("Sure you want to send verification code !"))
	{					
		$.ajax({	
			   type: "GET",
			   url: '<?= base_url()?>cms/adm-panel/send_email_verification/'+ id,
			   success: function(){
						alert('success !');
				   	}
    			});

...
//				load_template('admin','admin/add_edit', $this->data);
			load_template('admin', 'auth/index', $data) ;
		}
	}
		...
		if ($this->form_validation->run() == true)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember) && $this->ion_auth->is_admin())
			{
				...
				if($this->ion_auth->is_admin()){
					redirect('adm-panel/dashboard', 'refresh');	
				}else{
					redirect('adm-panel', 'refresh');		
				}			
			...
			else
			{
				...
				redirect('adm-panel', 'refresh'); //use redirects instead of loading views for compatibility with MY_Controller libraries
			}
		}
		else
		{
...
//			redirect('/', 'refresh');	
	...
	$user_row = $this->ion_auth->user()->row() ;	
		...
		$hashed_new_password  = $this->ion_auth_model->hash_password($this->input->post('newpassword'), $user_row->salt);
			...
			$this->db->update('users', $data, array('email' => $user_row->email));
		}
	
	 	load_template('admin', 'auth/change_password'); 	
	} 
...
//			$this->_render_page('auth/change_password', $data);
			load_template('admin', 'auth/change_password', $data) ;
		}
		else
		{
...
//			$this->_render_page('auth/forgot_password', $data);
			load_template('admin', 'auth/forgot_password', $data) ;
		}
		else
		{
...
//				$this->_render_page('auth/reset_password', $data);
			load_template('admin', 'auth/reset_password', $data) ;
			}
			else
			{
...
//			redirect("auth/forgot_password", 'refresh'); 
	...
	function deactivate($id = NULL)
	{
		...
		load_template('admin', 'auth/deactivate_user', $data) ;
		}
		else
		{
	...
	function create_user()
	{
		...
		//	$this->_render_page('auth/create_user', $data);
				load_template('admin', 'auth/create_user', $data) ;
		}
	}
...
//		$this->_render_page('auth/edit_user', $data); 
			load_template('admin', 'auth/edit_user', $data) ;
	}
...
//			$this->_render_page('auth/create_group', $data);
			load_template('admin', 'auth/create_group', $data) ;
...
//		$this->_render_page('auth/edit_group', $data);
			load_template('admin', 'auth/edit_group', $data) ;

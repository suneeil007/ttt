...
<?php echo $this->load->view('front/segments/headerPS')?>
...
<ul class="ceebox html">
    ...
    <li>
		...
		<a href="<?php echo base_url().'tour/'.decode_title(strtolower($product->name)).'/'.$product->id ;?>"><h4><?php echo $product->name ;?></h4></a>
...
<h3 style="border-bottom:1px solid #eee; padding-bottom:15px;"><?php if(isset($data_cms->title)){ echo $data_cms->title ; }elseif(isset($current_category)) { echo $current_category->name; }elseif($this->uri->segment(2)=='search') echo 'Search Results'?></h3>
		...
		<a href="<?php echo base_url().'tours/'.decode_title(strtolower($row->name)).'/'.$row->id ;?>"><h4><?php echo $row->name;?></h4></a>
		<p class="desccription"><?php  echo word_limiter($row->description, 60);?></p>
	<!--	<p class="time">4 days 5 nights</p>
		<p class="time">$ 5000</p>-->
		
		
<p class="time1"><a href="<?php echo base_url().'tours/'.decode_title(strtolower($row->name)).'/'.$row->id ;?>" style="height:45px; width:155px;  display:block; float:right; line-height:44px;">
<button type="button" class="btn btn-info">View All</button></a></p>

...
$.fn.ceebox.defaults = {
	...
	borderWidth: "8px", //the border on ceebox. Can be used like css ie,"4px 2px 4px 2px"
...
$.fn.ceebox.popup = function(content,opts) {
	...
	if ($(content).is("a,area,input") && (opts.type == "html" || opts.type == "image" || opts.type == "video")) { //
		...
		opts.width = 1030;
		opts.height = 575 + opts.titleHeight + 2*opts.padding;
	}
...
var BoxAttr = function(cblink,o) {
	...
	this.titlebox = (o.titles) ? "<div id='cee_title'><h2></h2></div>" : "";
	this.width = w;
	this.height = h;
	this.rel = rel;
...
var Build = {
	...
	html: function() {
		...
		if ((document.domain == m[0] && m[1] && !m[2]) || (!m[1] && !m[2])) { //if linked to same domain and not iframe than it's an ajax link
			...
			this.content = this.titlebox + "<div id='cee_ajax' style='width:"+(970)+"px;height:"+(560)+"px'></div>";
		} else {
			$("#cee_iframe").remove();
			this.content = this.titlebox + "<iframe frameborder='0' hspace='0' src='"+h+"' id='cee_iframeContent' name='cee_iframeContent"+Math.round(Math.random()*1000)+"' onload='jQuery.fn.ceebox.onload()' style='width:"+(1000)+"px;height:"+(565)+"px;' > </iframe>";
